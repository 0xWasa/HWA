...
library BLS {
    ...
    // The generic ECADD helper from the archived upstream library was deliberately removed: it
    // was unused by the verifier and supplied incorrect precompile memory lengths.
    ...
    // The generic ECMUL helper was also unused and removed for the same reason. The verifier only
    // uses the separately bounded base-point multiplication routine below.
    ...
    // No public-key validation helper is exposed: an on-curve check without a subgroup check is
    // unsafe for untrusted keys. HWA's audited evmnet key is a compile-time constant.
    ...
    // The archived `isValidPublicKey` helper was removed because it did not perform a G2 subgroup
    // check and was not part of HWA's fixed-key verification path.
    ...
    function expandMsg(bytes memory DST, bytes memory message, uint8 n_bytes) internal pure returns (bytes memory) {
        require(n_bytes != 0, "Invalid expand length");

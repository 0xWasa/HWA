...
10b. Republish the subgraph against the new core and repoint the frontend at it. Skipping this is not a degraded state, it is a silent one: every listing id still resolves on both cores, so the pool renders v1 names against v2 economics with no error anywhere. Before repointing, prove the deployment by comparing one listing against the chain (collection, tokenId and backing must match) and by checking that the indexed active count equals `activeListingCount()`. Point the frontend at the `mainnet` tag rather than a pinned version, so deleting an old version cannot strand production. Ownership indexing must start from each collection's real deployment block: a shortened window leaves long-term holders unable to see their own NFTs in the deposit picker.

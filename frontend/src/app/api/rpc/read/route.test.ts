...
describe("read RPC proxy", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    delete process.env.HYPEREVM_READ_RPC_UPSTREAM_URL;
    delete process.env.HYPEREVM_READ_RPC_API_KEY;
  });
  ...
  it("fails closed without a server upstream", async () => {
    ...
    process.env.HYPEREVM_LOG_RPC_UPSTREAM_URL = "https://archive.example.test";
  });

  it("rejects writes, log scans, malformed calls and oversized fee histories", async () => {
    process.env.HYPEREVM_READ_RPC_UPSTREAM_URL = "https://rpc.example.test";
  ...
  it("forwards bounded read batches without exposing the credential", async () => {
    process.env.HYPEREVM_READ_RPC_UPSTREAM_URL = "https://rpc.example.test/v2/private";
    process.env.HYPEREVM_READ_RPC_API_KEY = "secret-test-key";
    ...
    const cached = await POST(request([
      { ...call("eth_chainId"), id: 17 },
      { ...call("eth_getBalance", ["0x1111111111111111111111111111111111111111", "latest"]), id: 18 },
    ]));
    expect(cached.status).toBe(200);
    expect(await cached.json()).toEqual([
      { jsonrpc: "2.0", id: 17, result: "0x3e7" },
      { jsonrpc: "2.0", id: 18, result: "0x1" },
    ]);

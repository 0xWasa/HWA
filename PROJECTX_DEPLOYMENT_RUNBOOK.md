...
- owner final = Safe contract, jamais l'EOA de déploiement ;
...
4. Binding core : action Safe `FWA.setRewards` produite par `PrepareMainnetOwnerActions.ps1`.
...
`BindProjectXRewards.s.sol` est volontairement limité au testnet avec owner EOA et refuse la chain
999. En mainnet, l'owner FWA est le Safe 2-of-3 : aucune clé privée owner n'existe. Générer, relire
et exécuter la transaction `preActivation` du Safe décrite dans le runbook mainnet.
...
Gameplay et trading sont indépendants. Le Safe active d'abord le canary gameplay. Les achats Project X ne sont ouverts que par une action ultérieure, manuelle et réversible. Aucune activation n'est couplée à une heure ou à un nombre de blocs.
